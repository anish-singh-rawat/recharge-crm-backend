...
export const rechargePlanService = {
  async getPlans(operatorId, circleId) {
    ...
    if (!operator || !operator.isActive) {
      throw new NotFoundError('Operator not found or inactive');
    }
    if (!circle || !circle.isActive) {
      throw new NotFoundError('Circle not found or inactive');
    }
    ...
    try {
      rawPlans = await mroboticsProvider.getPlans({ operatorCode, circleCode });
      providerLogger.info('Plans fetched from MRobotics', {
        operatorCode,
        circleCode,
        count: Array.isArray(rawPlans) ? rawPlans.length : 0,
      });
    } catch (err) {
  ...
  async validateAmount(amount, operatorId, circleId) {
    ...
    if (plans.length === 0) {
      ...
      return { valid: true, plan: null, noPlansAvailable: true };
    }
    const matched = plans.find((p) => Number(p.amount) === Number(amount));
    return { valid: !!matched, plan: matched || null };
  },
  ...
  async _enrichWithPopularity(plans, operatorId) {
    ...
    return plans.map((p) => ({
      ...p,
      isPopular: popularAmounts.has(Number(p.amount)),
    }));
  },
};
